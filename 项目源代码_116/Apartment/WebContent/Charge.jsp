...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span></span><span>账单基本信息管理</span></div>
        </div>
        
     <!-- 查询 -->   
         <div class="search-wrap">
            <div class="search-content">
                <form action="ContractServlet?Charge=1" method="post">
        ...
        <div class="result-wrap">
            <form  action="ContractServlet?Charge=alldelete" name="myform" id="myform" method="post">
                   ...
                   <%
					...
					<jsp:useBean id="cdb" scope="page" class="com.util.ConnectDB"></jsp:useBean>
                       ...
                       <%
						...
						%>
						<%float eleCharge=(float)session.getAttribute("eleCharge");
						float waterCharge=(float)session.getAttribute("waterCharge");%>
                     	  <%
                        ...
                        <tr>              				        				
                            ...
                            <td><%=result.getString("isCost") %></td> 
                            
                            <%if(result.getString("isCost").equals("未付款")){ %>
                             <td><%=new RoomDaoimpl().getRoomCharge(String.valueOf(result.getString("roomID"))) %>+<%=result.getInt("eleDegree")*eleCharge %>+<%=result.getInt("waterDegree")*waterCharge %></td>                     
                            <td>                                           
                                <a class="link-del" href="ConfirmChargeServlet?roomID=<%=result.getString("roomID")%>"  onclick="return confirm('确定住户已付款?');"><i class="require-red">确认付款</i></a>
                            </td> 
                            <%}else{ %>    
							<td>0</td>
		     				<td>                                           
                    ...
                    <div class="black2">
						...
						<%}else{%>
							<a href="Charge.jsp?ToPage=<%=ShowPage-1 %>"> < </a>
						<%} %>
						<%for(int i=1;i<=PageCount;i++) {%>	
						<%if(ShowPage==i) {%>
							<span class="current"><%=i %></span>
							<%}else{%>			
							<a  href="Charge.jsp?ToPage=<%=i%>"><%=i%></a>
							<%} %>
						<%} %>	
						<%if(ShowPage==PageCount){ %>						
							<span class="disabled"> > </span>
