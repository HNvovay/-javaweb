...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span></span><span>公寓基本信息管理</span></div>
        </div>
     ...
     <!-- 查询 -->   
         <div class="search-wrap">
            <div class="search-content">
                ...
                <input type="hidden" name="Room" value="check">
                    <table class="search-tab">
                        <tr>
                            <th width="150">请输入公寓ID或姓名:</th>
                            <td><input class="common-text" placeholder="such as:102" name="checkroom" type="text"></td>
        ...
        <div class="result-wrap">
            <form  action="RoomServlet?Room=alldelete" name="myform" id="myform" method="post">
            
            <!-- 功能 -->
                <div class="result-title">
                    <div class="result-list">
                        <a href="addRoom.jsp"><i class="icon-font"></i>新增公寓</a>
                        ...
                        <a id="updateOrd" href="room.jsp"><i class="icon-font"></i>刷新</a>
                    </div>
                </div>
                ...
                <div class="result-content">
                   ...
                   <%
					...
					<jsp:useBean id="cdb" scope="page" class="com.util.ConnectDB"></jsp:useBean>
							...
							conn=cdb.getConn();
							System.out.println();
							 String sql = "select * from room order by roomID+0 ASC";
                       ...
                       <%
						...
						%>
                     	  <%
							...
							%>               			
                        <tr>
               <jsp:useBean id="roomdaoimpl" scope="page" class="com.daoimpl.RoomDaoimpl"></jsp:useBean>
                            <td class="tc"><input type="checkbox" name="checkAll" id="checkAll" onclick="setSelectAll();" value="<%=result.getString("roomID")%>"/></td>                          
                            <td><%=result.getString("roomID") %></td>
                            <td><%=roomdaoimpl.getroomStyle(result.getString("roomStyle")) %></td>
                            <td><%=result.getString("roomStroey") %></td>                        
                            <td>
                                <a class="link-del" href="updateroommessage.jsp?roomID=<%=result.getString("roomID")%>">修改</a> 
                                <a class="link-del" href="RoomServlet?roomID=<%=result.getString("roomID")%>&Room=delete"  onclick="return confirm('确定删除?');">删除</a>
                            </td>
                        </tr>
                    ...
                    <div class="black2">
						...
						<%}else{%>
							<a href="room.jsp?ToPage=<%=ShowPage-1 %>"> < </a>
						<%} %>
						<%for(int i=1;i<=PageCount;i++) {%>	
						<%if(ShowPage==i) {%>
							<span class="current"><%=i %></span>
							<%}else{%>			
							<a  href="room.jsp?ToPage=<%=i%>"><%=i%></a>
							<%} %>
						<%} %>	
						<%if(ShowPage==PageCount){ %>						
							<span class="disabled"> > </span>
