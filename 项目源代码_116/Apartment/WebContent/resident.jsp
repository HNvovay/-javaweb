...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="index.jsp">首页</a><span class="crumb-step">&gt;</span></span><span>用户基本信息管理</span></div>
        </div>
     ...
     <!-- 查询 -->   
            ...
            <div class="search-content">
                <form action="ResidentServlet" method="post">
                    <table class="search-tab">
                        <tr>
                            <th width="150">请输入用户ID或姓名:</th>
                       
                            <td><input class="common-text" placeholder="such as:张三" name="checkresident" type="text"></td>
                            <td>
                            <input type="hidden" name="Resident" value="check">
        ...
        <div class="result-wrap">
            <form  action="ResidentServlet" name="myform" id="myform" method="post">
            <input type="hidden" name="Resident" value="alldelete">
            <!-- 功能 -->
                <div class="result-title">
                    <div class="result-list">
                        <a href="addResident.jsp"><i class="icon-font"></i>新增用户</a>
                        ...
                        <a id="updateOrd" href="resident.jsp"><i class="icon-font"></i>刷新</a>
                    </div>
                </div>
                ...
                <div class="result-content">
                   ...
                   <%
					...
					<jsp:useBean id="cdb" scope="page" class="com.util.ConnectDB"></jsp:useBean>
                       ...
                       <%
                        ...
                        <tr>
                            ...
                            <td class="tc"><input type="checkbox" name="checkAll" id="checkAll" onclick="setSelectAll();" value="<%=result.getString("residentID")%>"/></td>                           
                            <td><%=result.getString("residentID") %></td>
                            <td><%=result.getString("residentName") %></td>
                            <td><%=result.getString("residentAge") %></td>
                            <td><%=result.getString("residentNumber") %></td>
                            <td><%=result.getString("residentProfession") %></td>
                            <td><%=result.getString("residentIDCardNum") %></td>
                            <td><%=result.getString("remarks") %></td>

                            <td>
                                <a class="link-del" href="updateresidentmessage.jsp?residentID=<%=result.getString("residentID")%>">修改</a> 
                                <a class="link-del" href="ResidentServlet?residentID=<%=result.getString("residentID")%>&Resident=delete" onclick="return confirm('确定删除?');">删除</a>
                            </td>
                        </tr>
                    ...
                    <div class="black2">
						...
						<%}else{%>
							<a href="resident.jsp?ToPage=<%=ShowPage-1 %>"> < </a>
						<%} %>
						<%for(int i=1;i<=PageCount;i++) {%>	
						<%if(ShowPage==i) {%>
							<span class="current"><%=i %></span>
							<%}else{%>			
							<a  href="resident.jsp?ToPage=<%=i%>"><%=i%></a>
							<%} %>
						<%} %>	
						<%if(ShowPage==PageCount){ %>						
							<span class="disabled"> > </span>
